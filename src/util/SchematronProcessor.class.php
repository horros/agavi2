...
class SchematronProcessor extends ParameterHolder
{
	...
	public function __construct(array $chain = null)
	{
		...
		if(!$chain) {
			throw new \Exception('Schematron processor chain must contain at least one path name.');
		}
		
		$this->chain = array_map(array('Agavi\\Util\\Toolkit', 'expandDirectives'), $chain);
	}
