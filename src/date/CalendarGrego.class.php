...
final class CalendarGrego
{
	 ...
	 * 
	 * @param      int $year Gregorian year, with 0 == 1 BCE, -1 == 2 BCE, etc.
	 * 
	 ...
	 *
	 * @param      int $year Gregorian year, with 0 == 1 BCE, -1 == 2 BCE, etc.
	 * @param      int $month Month 0-based month, with 0==Jan
	 * 
	 ...
	 * 
	 * @param      int $y The extended year
	 * @param      int $m The 0-based month number
	 ...
	 * @param      int   $dom 1-based day of month
	 * 
	 ...
	 * 
	 * @param      float $day Day 1970-epoch day (integral value)
	 * @param      int   $year Output parameter to receive year
	 * @param      int   $month Output parameter to receive month (0-based, 0==Jan)
	 * @param      int   $dom Output parameter to receive day-of-month (1-based)
	 * @param      int   $dow Output parameter to receive day-of-week (1-based, 1==Sun)
	 * @param      int   $day Output parameter to receive day-of-year (1-based)
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	public static function dayToFields($day, &$year, &$month, &$dom, &$dow, &$doy = 0)
	{
		...
		// 1461 == 365*4 + 1 days.
		$n400 = (int) Toolkit::floorDivide($day, 146097, $doy); // 400-year cycle length
		$n100 = (int) Toolkit::floorDivide($doy, 36524, $doy); // 100-year cycle length
		$n4   = (int) Toolkit::floorDivide($doy, 1461, $doy); // 4-year cycle length
		$n1   = (int) Toolkit::floorDivide($doy, 365, $doy);
		...
		$dow += ($dow < 0) ? (DateDefinitions::SUNDAY + 7) : DateDefinitions::SUNDAY;
	 ...
	 * 
	 * @param      float  $time 1970-epoch milliseconds
	 * @param      int    $year output parameter to receive year
	 * @param      int    $month output parameter to receive month (0-based, 0==Jan)
	 * @param      int    $dom output parameter to receive day-of-month (1-based)
	 * @param      int    $dow output parameter to receive day-of-week (1-based, 1==Sun)
	 * @param      int    $doy output parameter to receive day-of-year (1-based)
	 * @param      int    $mid output parameter to recieve millis-in-day
	 * 
	...
	{
		$millisInDay = null;
		$day = Toolkit::floorDivide($time, DateDefinitions::MILLIS_PER_DAY, $millisInDay); // 400-year cycle length
		$mid = (int) $millisInDay;
	 ...
	 * 
	 * @param      float  $day day the 1970-epoch day (integral value)
	 * 
	...
	{
		$dow = null;
		Toolkit::floorDivide($day + DateDefinitions::THURSDAY, 7, $dow);
		return ($dow == 0) ? DateDefinitions::SATURDAY : $dow;
	}
	 ...
	 * 
	 * @param      int   $julian The given Julian day number.
	 * 
