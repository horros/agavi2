...
class UploadedFile implements \ArrayAccess
{
    ...
    public function __construct(array $array)
    {
        ...
        if (isset($array['tmp_name'], $array['contents'], $array['stream']) ||
    ...
    public function getTmpName()
    {
        if (!$this->hasTmpName() && !$this->hasOpenStream()) {
            ...
            $this->stream = tmpfile();
            if (!$this->stream) {
                throw new FileException('Cannot create temporary file via tmpfile()');
            }
     ...
     *
     * @throws     Exception If the file has errors or has been moved.
     *
     * @author     David Zülke <dz@bitxtender.com>
    ...
    public function getContents()
    {
        ...
        if ($this->hasError() || (!$this->isMovable() && !$this->hasBufferedContents() && !$this->hasOpenStream())) {
            throw new Exception('Cannot get contents of erroneous or moved file.');
        }
     ...
     *
     * @throws     FileException If chmod or mkdir calls failed.
     *
     * @author     David Zülke <dz@bitxtender.com>
     * @since      0.11.0
     */
    public function move($dest, $fileMode = 0664, $create = true, $dirMode = 0775)
    {
        ...
        if (!is_readable($directory)) {
            if ($create && !Toolkit::mkdir($directory, $dirMode, true)) {
        ...
        if ($moved) {
            ...
            if (!@chmod($dest, $fileMode)) {
                throw new FileException('Failed to chmod uploaded file after moving');
            }
        } else {
