...
class HttpRedirectRoutingCallback extends RoutingCallback
{
	...
	public function initialize(Context $context, array &$route)
	{
		...
		if(!($context->getRouting() instanceof WebRouting)) {
			throw new AgaviException('HttpRedirectRoutingCallback can only be used in combination with WebRouting.');
		}
	}
	...
	public function onMatched(array &$parameters, ExecutionContainer $container)
	{
		...
		if($this->hasParameter('route')) {
			...
			array_walk_recursive($arguments, function(&$argument) use($parameters) { $argument = Toolkit::expandVariables($argument, $parameters); });
			...
			if($parts) {
				$url = Toolkit::buildUrl(array_merge(parse_url($this->getContext()->getRequest()->getUrl()), $parts));
			} else {
		...
		if(!($response instanceof WebResponse)) {
			throw new AgaviException('HttpRedirectRoutingCallback can only be used in combination with AgaviWebResponse.');
		}
