...
require_once(__DIR__ . '/OptionException.class.php');
...
class OptionParser
{
	 ...
	 *
	 * @param      string $option The short option name.
	 *
	 ...
	 *
	 * @param      string $option The long option name.
	 *
	 * @return     string The value that must be matched in the source data.
	 *
	 * @author     Noah Fontes <noah.fontes@bitextender.com>
		...
		for($i = 0; $i < $size; $i++) {
			...
			if($handler === null) {
				if(strpos($source[$i], $this->shortNamePrefix) === 0 ||
					...
					throw new OptionException(sprintf('Unexpected option %s', $source[$i]));
				} else {
					/* Accept arguments. */
					break;
				}
	 ...
	 *
	 * @param      string $name A unique identifier for the option.
	 * @param      array  $shortNames The short names (often characters) that identify the
	 *                    option in source data.
	 * @param      array  $longNames The long names that identify the option in source data.
	 * @param      int    $arguments The number of options that this option expects.
	 *
	 * @author     Noah Fontes <noah.fontes@bitextender.com>
	 * @since      1.0.0
	 */
